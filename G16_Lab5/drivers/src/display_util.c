...
inline void clear_signal(int pixel_vector[VGA_COL_LEN], 
                         int current_pixel_vector[VGA_COL_LEN]) {
    int i = 0, j = 0;
...
inline void render(int volume, char is_pressed[]) {
    int i; 
    ...
    for (i = 0; i < 7; i++) {
        if (i < 5) 
            screen_write_cstring((char []){'L', i + 'C', ':', 
                                           is_pressed[i] + '0', 
                                           ' ', '\0'}, 45 + i * 5, 4);
        else 
            screen_write_cstring((char []){'L', i - 5 + 'A', ':', 
                                           is_pressed[i] + '0', 
                                           ' ', '\0'}, 45 + i * 5, 4);
    }
    for (i = 7; i < NUM_NOTES; i++) {
        if (i < 12) screen_write_cstring((char []){'C', i - 7 + 'C', ':', 
                                                   is_pressed[i] + '0', 
                                                   ' ', '\0'}, 
                                                   45 + (i - 7) * 5, 6);
        else if (i == NUM_NOTES - 1) 
            screen_write_cstring((char []){'HH', 'C', ':', 
                                           is_pressed[i] + '0', 
                                           ' ', '\0'}, 75, 8);
        else 
            screen_write_cstring((char []){'C', i - 12 + 'A', ':', 
                                           is_pressed[i] + '0', 
                                           ' ', '\0'}, 45 + (i - 7) * 5, 6);
    }
}

inline void screen_write_cstring(char cstr[], int sx, int sy) {
    int i = 0;
    while (cstr[i] != '\0') {
